...
//            ('id'), ('id, name')
    ...
    public function insert($insertDefinition = null)
    {
        if (!is_array($insertDefinition) || !count($insertDefinition)) {
            throw new InvalidArgumentException(
                'The parameter for the insert function should be a nono-empty associative array'
            );
        }
        ...
        foreach ($insertDefinition as $col => $value) {
            if (is_numeric($col)) {
                throw new InvalidArgumentException(
                    'Only associative array is allowed as parameter for the insert function'
                );
            }
    ...
    private function W($term, $col, $mod, $val)
    {
        ...
        if (is_callable($col)) {
            ...
            $subQuery->setGrammar($this->grammar);
            $col($subQuery);
